...
func (this *DbAdapter) SearchByEmail(email string) (d.User, error) {
	query := `SELECT id, email, first_name, last_name, dob,gender from users WHERE email = $1`
...
func (this *DbAdapter) CreateUser(u d.User) error {
	query := getInsertQuery(`users`)
	query = query + ` VALUES ($1, $2, $3, $4, $5, $6, $7)`

	_, err := this.Connection.Exec(query, u.FirstName, u.LastName, u.Email, u.Gender, u.PasswordHash(), time.Now(), time.Now())
	return err
}
